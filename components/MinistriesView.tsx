...
import { 
  ...
  Activity, Scale, Milestone, HardHat, Landmark
} from 'lucide-react';
import { Ministry, Program } from '../types';
...
const allMinistries: Ministry[] = [
  {
    ...
    budget2026: 32.54,
    investmentShare: 44,
    executionRate: 78.5, // Budgetary Execution
    physicalRate: 62.0, // Physical Execution
    programs: [
      {
        id: 'P1',
  ...
  },
  {
    ...
    minister: "Dr. Ibrahima Sy",
    totalBudgetTriennal: 380.4,
    budget2026: 120.5,
    investmentShare: 40,
    executionRate: 72.0,
    physicalRate: 58.0,
...
const MinistriesView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'programs' | 'budget' | 'performance'>('programs');
    ...
    return (
      <div className="space-y-10 animate-in slide-in-from-right duration-300 pb-16">
        ...
        <div className="bg-white rounded-[3.5rem] shadow-premium border border-slate-100 overflow-hidden">
          ...
          <div className="state-gradient p-14 text-white relative">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-10 relative z-10">
              <div className="flex items-center">
                <div className="w-28 h-28 bg-white/10 backdrop-blur-xl rounded-[2.5rem] flex items-center justify-center mr-10 border border-white/20 shadow-2xl group transition-all">
                  <Anchor size={56} className="text-accent-gold group-hover:scale-110 transition-transform" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-6 min-w-[320px]">
                ...
                <div className="bg-white/5 backdrop-blur-xl rounded-[2rem] p-6 border border-white/10 text-center">
                   <div className="text-[9px] uppercase font-black tracking-widest text-accent-gold mb-1">Nombre d'Agents</div>
                   <div className="text-3xl font-serif font-bold">12 450</div>
                </div>
              </div>
            </div>
          ...
          <div className="p-14">
            <div className="flex space-x-12 mb-14 border-b border-slate-100">
              {[
                { id: 'programs', label: 'Programmes & Projets' },
                { id: 'budget', label: 'Analyse Budgétaire' },
                { id: 'performance', label: 'Dashboard Stratégique' }
              ].map((tab) => (
                <button 
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as any)}
                  className={`pb-6 text-[11px] font-black uppercase tracking-[0.2em] transition-all border-b-4 ${activeTab === tab.id ? 'border-emerald-600 text-state-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                  {tab.label}
                </button>
              ))}
              ...
              <div className="space-y-12 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                   {[
                     ...
                     { label: 'Efficience Coût', val: '0.92', icon: Scale, color: 'blue' },
                     { label: 'Progrès Physique', val: '+5.4%', icon: Milestone, color: 'amber' },
                     { label: 'Équité Territ.', val: 'A-', icon: Globe, color: 'purple' },
                     ...
                     <div key={i} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:shadow-xl transition-all">
                        <div className={`w-14 h-14 bg-${kpi.color}-50 text-${kpi.color}-600 rounded-2xl flex items-center justify-center mb-6 border border-${kpi.color}-100 group-hover:scale-110 transition-transform`}>
                           <kpi.icon size={28} />
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                   <div className="lg:col-span-8 bg-white p-12 rounded-[3rem] border border-slate-200 shadow-premium">
                      <h4 className="font-serif font-bold text-2xl mb-10 flex items-center text-state-900">
                         <Users size={24} className="mr-4 text-emerald-700" /> Analyse RH & Masse Salariale
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                         <div className="space-y-8">
                            <div>
                               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Masse Salariale Précise</div>
                               <div className="text-3xl font-black text-state-900">12.45 <span className="text-sm font-sans opacity-30 italic">Mds</span></div>
                            </div>
                            <div>
                               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Part du Budget Global</div>
                               <div className="text-3xl font-black text-emerald-700">38.2%</div>
                            </div>
                         </div>
                         <div className="md:col-span-2 bg-slate-50/50 p-8 rounded-3xl border border-slate-100">
                            <h5 className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-6">Répartition par Hiérarchie</h5>
                            <div className="space-y-4">
                               {[
                                 { h: 'Hiérarchie A', val: 1200, p: 45 },
                                 { h: 'Hiérarchie B', val: 3400, p: 30 },
                                 { h: 'Hiérarchie C', val: 5600, p: 20 },
                                 { h: 'Hiérarchie D', val: 2250, p: 5 },
                               ].map((hi, i) => (
                                 <div key={i} className="flex items-center">
                                    <span className="w-32 text-[10px] font-black text-slate-400 uppercase">{hi.h}</span>
                                    <div className="flex-1 h-2.5 bg-white rounded-full overflow-hidden border border-slate-100 mx-4">
                                       <div className="state-gradient h-full" style={{width: `${hi.p}%`}}></div>
                                    </div>
                                    <span className="text-[11px] font-bold text-state-900">{hi.val}</span>
                                 </div>
                               ))}
                            </div>
                         </div>
                   ...
                   <div className="lg:col-span-4 bg-state-900 p-10 rounded-[3rem] text-white shadow-2xl flex flex-col justify-between">
                      <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-accent-gold mb-10">Score de Performance Global</h4>
                      <div className="text-8xl font-serif font-bold text-center mb-8">82<span className="text-2xl opacity-30">/100</span></div>
                      <div className="p-6 bg-white/5 rounded-3xl border border-white/5 text-center">
                         <div className="text-[10px] font-bold uppercase text-emerald-400 mb-2">Note Parlementaire</div>
                         <div className="text-xl font-serif font-bold">A- (Solide)</div>
                      </div>
                      <p className="text-[11px] text-center text-emerald-100 italic mt-8 leading-relaxed">
                         "Gestion exemplaire des ressources internes, amélioration attendue sur l'équité territoriale."
                      </p>
                   </div>
                </div>
              </div>
                ...
                {selectedMinistry.programs.map(program => (
                  <div key={program.id} onClick={() => setSelectedProgram(program)} className="group bg-white border border-slate-200 rounded-[2.5rem] p-10 hover:shadow-premium hover:border-emerald-200 transition-all cursor-pointer">
                    ...
                    <div className="flex items-center text-[10px] font-black text-state-900 uppercase tracking-widest group-hover:translate-x-3 transition-transform">
                       Voir les projets opérationnels <ArrowRight size={16} className="ml-3 text-accent-gold" />
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="page-transition space-y-12 pb-16">
      <div className="max-w-3xl">
         ...
         <p className="text-slate-500 mt-6 text-xl font-medium">Contrôle consolidé des 25 Départements Ministériels.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
        {[
          ...
          { label: "Score Performance", value: "A-", unit: "", icon: Target, color: "purple" }
        ].map((kpi, i) => (
          <div key={i} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-premium group hover:shadow-2xl transition-all">
             <div className={`w-16 h-16 bg-${kpi.color}-50 text-${kpi.color}-600 rounded-[1.5rem] flex items-center justify-center mb-8 border border-${kpi.color}-100 group-hover:scale-110 transition-transform`}>
               <kpi.icon size={32} />
             </div>
